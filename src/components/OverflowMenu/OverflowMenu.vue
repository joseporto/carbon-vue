<template>
  <div data-overflow-menu :class="`${carbonPrefix}--overflow-menu`" :id="uid">
    <button
      :class="[
    ...
    >
      ...
      <slot name="trigger">
        <Icon icon="OverflowMenuVertical" :class="`${carbonPrefix}--overflow-menu__icon`" />
      </slot>
    </button>
    <div
      :class="[
    ...
    >
      <div
        class="overflow-menu__before-content"
        ref="beforeContent"
        tabindex="0"
      ...
      <div
        class="overflow-menu__after-content"
        ref="afterContent"
        tabindex="0"
        style="position: absolute; height: 1px; width: 1px; left: -9999px;"
...
export default {
  name,
  mixins: [Uid, CarbonPrefix, Methods({ trigger: ['blur', 'focus'] })],
  props: {
    label: String,
    flipMenu: Boolean,
    up: Boolean,
  ...
  created() {
    ...
    this.$on('click', this.menuItemclick);
  },
  methods: {
    checkFocusOut(ev) {
      if (this.open) {
      ...
      if (this.open) {
        ...
        for (let tryOn of focusOnList) {
          if (
            ...
            !(
              tryOn.classList.contains('overflow-menu__before-content') ||
              tryOn.classList.contains('overflow-menu__after-content') ||
              tryOn.tabindex < 0
            )
          ) {
            focusOn = tryOn;
